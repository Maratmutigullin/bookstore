...
class AuthorBookChapterService
{
    ...
    public function createChapter(CreateBookChapterRequest $bookChapterRequest, int $bookId): IdResponse
    {
        $book = $this->bookRepository->getBookById($bookId);
        $title = $bookChapterRequest->getTitle();
        $parentId = $bookChapterRequest->getParentId();
        $parent = null;
        ...
        $chapter = (new BookChapter())
            ...
            ->setParent($parent)
            ->setLevel($level)
            ->setSort($this->getNextMaxStep($book, $level))
            ->setBook($book);
    ...
    private function getNextMaxStep(Book $book, int $level)
    {
    ...
    {
        //вернем сортированный список глав где в ключе будет id а в значении сама модель
        $book = $this->bookRepository->getBookById($bookId);
         ...
         * нужен для того чтобы по parent найти главу во вложенном списке
         */
        $index = [];
