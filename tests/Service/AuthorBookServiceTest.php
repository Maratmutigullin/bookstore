...
class AuthorBookServiceTest extends AbstractTestCase
{
    ...
    public function testDeleteBook(): void
    {
        ...
        $this->createService()->deleteBookById(1);

    }

    public function testBook(): void
    {
        $categories = MockUtils::createBookCategory();
        $this->setEntityId($categories, 1);

        $format = MockUtils::createBookFormat();
        $this->setEntityId($format, 1);

        $book = MockUtils::createBook()->setCategories(new ArrayCollection([$categories]));
    ...
    public function testUpdateBookException(): void
    {
        $this->expectException(BookAlreadyExistsException::class);
